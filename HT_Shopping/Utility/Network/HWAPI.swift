...
public protocol HWAPIProtocol {
    ...
    //
    var baseHeader : [String:String]? { get set}
}
...
public extension HWAPIProtocol {
    ...
    func fetch(_ parameters: [String: Any]? = nil, headers: [String: String]? = nil) -> HWNetworkRequest {
        return HN.fetch(self, parameters: parameters, headers: headers)
    }
}
...
public extension HWNetworking {
    ...
    func fetch(_ api: HWAPIProtocol, parameters: [String: Any]? = nil, headers: [String: String]? = nil) -> HWNetworkRequest {
        ...
        var newheader : Dictionary<String, String>? = [String:String]()
        if let baseh = api.baseHeader {
            newheader?.merge(baseh)
        }
        if let h = headers {
            newheader?.merge(h)
        }
        let task = request(url: api.url, method: method, parameters: parameters, headers: newheader)
